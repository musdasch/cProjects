...
class SetSingleOptionsCprojectCommand(sublime_plugin.WindowCommand):
	...
	def run(self):
		...
		if 0 < len(self.getSettings()):
			self.input("single_options:", self.getSettings()["single_options"], self.setValue, None, None)
		else:
			self.error("There is no valid cProject.")

	def setValue(self, value):
		self.project_data["c_projects_settings"]["single_options"] = value
		
		self.window.set_project_data(self.project_data)
		self.message("New sigle_options: \"" + value + "\" - OK" )
