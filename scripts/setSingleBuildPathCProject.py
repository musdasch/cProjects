...
class SetSingleBuildPathCprojectCommand(sublime_plugin.WindowCommand):
	...
	def run(self):
		...
		if 0 < len(self.getSettings()):
			self.input("single_build_path:", self.getSettings()["single_build_path"], self.setBuildPath, None, None)
		else:
			self.error("There is no valid cProject.")

	def setBuildPath(self, path):
		self.project_data["c_projects_settings"]["single_build_path"] = path
