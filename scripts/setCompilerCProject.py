...
class SetCompilerCprojectCommand(sublime_plugin.WindowCommand):
	...
	def run(self):
		...
		if 0 < len(self.getSettings()):
			self.input("compiler:", self.getSettings()["compiler"], self.setValue, None, None)
		else:
			self.error("There is no valid cProject.")

	def setValue(self, value):
		self.project_data["c_projects_settings"]["compiler"] = value
		
		self.window.set_project_data(self.project_data)
		self.message("New Compiler: \"" + value + "\" - OK" )
